...
var setupFiles = function (config, projectDir, buildDir) {

  var buildSubDir = path.join(buildDir, 'build');
  ...
  shell.cp('-r', path.join(projectDir, config.build.appFolder, '*'), appDir);
  shell.cp(path.join(projectDir, 'package.json'), homeDir);
  shell.cp(path.join(projectDir, 'README.md'), homeDir);

  if (shell.test('-d', path.join(projectDir, 'config'))) {
    shell.mkdir('-p', path.join(homeDir, 'config'));
    shell.cp('-r', path.join(projectDir, 'config/*'), path.join(homeDir, 'config'));
  }

  if (shell.test('-f', './newrelic.js')) {
    shell.cp(path.join(projectDir, 'newrelic.js'), homeDir);
  }


  // move tpkg files to build location
  shell.mv(path.join(buildDir, 'tpkg.yml'), path.join(buildSubDir, 'tpkg.yml'));
  shell.mv(path.join(buildDir, 'postinstall'), path.join(buildSubDir, 'postinstall'));
  shell.mv(path.join(buildDir, 'postremove'), path.join(buildSubDir, 'postremove'));
  shell.mv(path.join(buildDir, 'crontab'),
    path.join(cronDir, config.project.name + '.crontab'));
  shell.mv(path.join(buildDir, 'logrotate.conf'),
    path.join(cronDir, config.project.name + '-logrotate.conf'));
  shell.mv(path.join(buildDir, 'init.sh'),
    path.join(initDir, config.project.name + '-init.sh'));
  shell.mv(path.join(buildDir, 'config.sh'),
...
var makeTpkg = function (config, buildDir) {
  ...
  if (!shell.test('-f', expectedFile)) {
    console.error('error - failed to create tpkg file');
    return false;
  }
